...
def dwt2d(input_node, wavelet, levels=1):
    ...
    for level in range(levels):
        ...
        second_pass = tf.transpose(
            dwt1d(tf.transpose(first_pass, perm=[1, 0, 2]), wavelet, 1), perm=[1, 0, 2]
        )
...
def idwt1d(input_node, wavelet, levels=1):
    ...
    Args:
        ...
        wavelet (nnfit.tfwavelets.dwtcoeffs.Wavelet):  Wavelet object.
    ...
    first_n = n // (2**levels)
    last_level = tf.slice(input_node, [0, 0, 0], [-1, first_n, 1])

    for level in range(levels - 1, -1, -1):
        local_n = n // (2**level)

        detail = tf.slice(input_node, [0, local_n // 2, 0], [-1, local_n // 2, 1])
...
def idwt2d(input_node, wavelet, levels=1):
    ...
    for level in range(levels - 1, -1, -1):
        ...
        detail_tr = tf.slice(
            input_node, [local_m // 2, 0, 0], [local_n // 2, local_m // 2, 1]
        )
        detail_bl = tf.slice(
            input_node, [0, local_n // 2, 0], [local_n // 2, local_m // 2, 1]
        )
        detail_br = tf.slice(
            input_node, [local_n // 2, local_m // 2, 0], [local_n // 2, local_m // 2, 1]
        )
        ...
        first_pass = tf.transpose(
            idwt1d(tf.transpose(this_level, perm=[1, 0, 2]), wavelet, 1), perm=[1, 0, 2]
        )
